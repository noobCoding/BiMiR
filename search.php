...
if ($result->num_rows <= 0) {	 
	...
	$display_string .="<th align='center' style='border: 1px solid black;'> Validated </th>";
	if (!$empty_tissue){
		$forward_post = "";
		if (isset($_POST['mirna'])){
		...
		<form method='post' action='searchp.php' class='inline'>".$forward_post.  
		"<button type='submit' name='submit_param' value='submit_value' class='link-button' style='text-decoration:none; color:black;font-family:Tahoma;font-size:16;font-weight:bold;'> Proportion </button></form>
	...
	while($row = $result->fetch_assoc()) {         
		$onerow = "<tr><td valign='top'>" 	. "<a href='showbc.php?src=j9as1dfhk234cuiihasdfvnkasdh000hkjasdfjkhe89045009345nksaaaajf232hkjasdjfaslkfjalsdf03209234jfllasdfjc09340945&add=".$row["Link"]."&nta=".$row["numTarget"]."&nco=".$row["numCondition"]."&fld=".$row["bimirFold"]."&pas=40zcvzafdaavcljluyinbz19asdfaf81zerqwr413kioiojuooyi41a23sdq54779erq114erqf2354a1239vbcn'>"								
										...
										// if ($odd_line%2 == 1){
											// $onerow .= 
										// } elseif ($odd_line%2 == 0) {
											// $onerow .= "</td><td valign='top' style='border: 1px solid black;'  bgcolor='#DDDDDD'>";											
										// }
       ...
       //// evidence
		...
		if (!$empty_disease || !$empty_tissue){
			...
			$onerow .= "<td align='left' valign='top' style='color:black;border: 1px solid black;'>"; // no proportion				
		...
		// } elseif ($odd_line%2 == 0) {			
			// $onerow .= $resTarget. "</td><td valign='top' style='border: 1px solid black;' bgcolor='#DDDDDD'>";						
		// }				
		// $odd_line ++;
		...
		////////////////////////////////////////////////////////
		// selection
		$row2sort["$onerow"] = $validatedScore;//$rate;
	}	
